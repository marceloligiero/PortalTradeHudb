...
export default function SubmissionReview() {
  ...
  const handleSaveClassification = async (operationId: number) => {
    try {
      ...
      // Verificar se foi auto-finalizado
      const subResp = await api.get(`/api/challenges/submissions/${submissionId}`);
  ...
  return (
    <div className="space-y-6">
      ...
      {(submission?.status === 'APPROVED' || submission?.status === 'REJECTED') && (
        <div className={`backdrop-blur-xl rounded-2xl border-2 p-6 ${
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <div className="mt-2 text-sm flex items-center gap-4">
                ...
                <span className="text-blue-400">MPU: {submission?.calculated_mpu?.toFixed(2) || '0.00'}</span>
              </div>
            </div>
            <button
              onClick={() => navigate(-1)}
            ...
            <div>
              ...
              {!isSummary && (
                <div className="mt-2 text-sm">
                  ...
                  <span className="text-red-400">{errorOperations.length} {t('submissionReview.withErrors').toLowerCase()}</span>
                  <span className="text-gray-500 mx-2">|</span>
                  <span className="text-yellow-400">{pendingReview.length} {t('submissionReview.pendingReview').toLowerCase()}</span>
                </div>
              )}
            </div>
            {/* Info badge */}
