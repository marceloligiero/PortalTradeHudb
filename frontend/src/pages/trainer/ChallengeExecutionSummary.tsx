        ...
        <div className="mb-8">
          ...
          <div className="mt-3 flex items-center gap-3">
            <div className="inline-block px-3 py-1 bg-yellow-500/10 border border-yellow-500/30 rounded-full">
              <span className="text-sm text-yellow-500 font-medium">{t('challenges.typeSummaryTitle', 'Desafio Resumido')}</span>
            </div>
            <div className="inline-block px-3 py-1 bg-white/5 border border-white/10 rounded-full text-sm text-gray-200">{t('submissionReview.maxErrors', 'Máx. Erros')}: {typeof challenge.max_errors !== 'undefined' ? challenge.max_errors : 0}</div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div className="bg-white/5 backdrop-blur-lg rounded-lg border border-white/10 p-4">
              <div className="flex items-center gap-2 text-gray-400 mb-1">
                ...
                <span className="text-sm">{t('challengeExecution.operationsTarget', 'Meta de Operações')}</span>
              </div>
            ...
            <div className="bg-white/5 backdrop-blur-lg rounded-lg border border-white/10 p-4">
              <div className="flex items-center gap-2 text-gray-400 mb-1">
                ...
                <span className="text-sm">{t('submissionReview.timeLimit', 'Tempo Limite')}</span>
              </div>
            ...
            <div className="bg-white/5 backdrop-blur-lg rounded-lg border border-white/10 p-4">
              <div className="flex items-center gap-2 text-gray-400 mb-1">
                ...
                <span className="text-sm">{t('submissionReview.targetMPU', 'Meta de MPU')}</span>
              </div>
        ...
        <div className="bg-white/5 backdrop-blur-lg rounded-xl border border-white/10 p-6 space-y-6">
          ...
          <div>
            ...
            {planStudent ? (
              ...
              >
                <option value="" className="bg-gray-800 text-white" style={{ backgroundColor: isDark ? '#0f0f14' : undefined }}>{t('placeholders.selectStudent')}</option>
                {students.map((student) => (
                  <option key={student.id} value={student.id} className="bg-gray-800 text-white" style={{ backgroundColor: isDark ? '#0f0f14' : undefined }}>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              <div className="flex items-center gap-2">
                ...
                {t('challengeExecution.totalOperationsPerformed', 'Total de Operações Realizadas')} *
              </div>
            </label>
            <input
              type="number"
              ...
              min="0"
            />
            <p className="text-xs text-gray-500 mt-1">{t('challengeExecution.operationsTarget', 'Meta')}: {challenge.operations_required} {t('challengeExecution.operations', 'operações')}</p>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              <div className="flex items-center gap-2">
                ...
                {t('challengeExecution.totalTimeMinutes', 'Tempo Total em Minutos')} *
              </div>
            </label>
            <input
              type="number"
              ...
              min="0"
            />
            <p className="text-xs text-gray-500 mt-1">{t('challengeExecution.limit', 'Limite')}: {challenge.time_limit_minutes} {t('challengeExecution.minutes', 'minutos')}</p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">{t('challengeExecution.referencesHint', 'Referências das operações realizadas (separe por vírgula se forem várias)')}</p>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              <div className="flex items-center gap-2">
                ...
                {t('challengeExecution.operationsWithErrors', 'Número de Operações com Erro')} *
              </div>
            </label>
            <input
              type="number"
            ...
            <p className={`text-xs mt-1 ${
              ...
              {t('challengeExecution.maxAllowed', 'Máximo permitido')}: {challenge.max_errors ?? 0} {t('challengeExecution.operationsWithErrorShort', 'operações com erro')}
              {formData.operations_with_errors > (challenge.max_errors ?? 0) && ` (${t('challengeExecution.exceeded', 'EXCEDIDO!')})`}
            </p>
          </div>
            ...
            <div className={`rounded-lg p-6 border-2 ${
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-gray-300">{t('submissionReview.calculatedMPU', 'MPU Calculado')}</span>
                  </div>
                ...
                <div className="text-right">
                  <p className={`text-sm font-medium ${approvalStatus?.isApproved ? 'text-green-500' : 'text-red-500'}`}>
                    {approvalStatus?.isApproved ? `✅ ${t('submissionReview.approved', 'APROVADO')}` : `❌ ${t('submissionReview.rejected', 'REPROVADO')}`}
                  </p>
              ...
              <div className="bg-white/5 rounded-lg p-3">
                <p className="text-sm text-gray-400">
                  {t('challengeExecution.calculation', 'Cálculo')}: {formData.total_time_minutes} {t('challengeExecution.minutes', 'minutos')} ÷ {formData.total_operations} {t('challengeExecution.operations', 'operações')}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  {t('challengeExecution.approvalTarget', 'Meta para aprovação')}: ≤ {challenge.target_mpu.toFixed(2)} min/op
                </p>
              </div>
            </div>
          )}
          ...
          <div className="bg-white/5 rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-white">{t('challengeExecution.errorsMade', 'Erros Cometidos')}</h3>
                <span className="text-xs text-gray-400 ml-2">
                  ({errorDetails.length} {t('challengeExecution.errorsRegistered', 'erro(s) registado(s)')})
                </span>
              </div>
              <button
                type="button"
              ...
              >
                ...
                {t('submissionReview.addError', 'Adicionar Erro')}
              </button>
            </div>

            {formData.operations_with_errors === 0 ? (
              <p className="text-gray-400 text-sm text-center py-4">
                {t('challengeExecution.informErrorsFirst', 'Informe o número de "Operações com Erro" acima para poder registar os erros detalhados.')}
              </p>
            ) : errorDetails.length === 0 ? (
              <p className="text-gray-400 text-sm text-center py-4">
                {t('challengeExecution.noErrorsRegistered', 'Nenhum erro registado. Clique em "Adicionar Erro" para registar os erros cometidos.')}
              </p>
            ) : (
                ...
                {errorDetails.map((err, index) => (
                  <div key={err.id} className="bg-white/5 rounded-lg p-4 border border-white/10">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm font-medium text-gray-300">{t('challengeExecution.errorNumber', 'Erro')} #{index + 1}</span>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        <label className="block text-xs text-gray-400 mb-1">{t('submissionReview.errorType', 'Tipo de Erro')} *</label>
                        <select
                        ...
                        >
                          {ERROR_TYPES.map(type => (
                            <option key={type.value} value={type.value} className="bg-gray-800 text-white" style={{ backgroundColor: isDark ? '#0f0f14' : undefined }}>
                              {t(type.label)} - {t(type.description)}
                            </option>
                          ))}
                        </select>
                      </div>
                      <div>
                        <label className="block text-xs text-gray-400 mb-1">{t('submissionReview.reference', 'Referência')} *</label>
                        {formData.operation_reference && formData.operation_reference.includes(',') ? (
                          <select
                            ...
                            style={{ backgroundColor: isDark ? '#0f0f14' : undefined }}
                          >
                            <option value="" className="bg-gray-800 text-white" style={{ backgroundColor: isDark ? '#0f0f14' : undefined }}>{t('submissionReview.selectReference', 'Selecione a referência')}</option>
                            {formData.operation_reference.split(',').map((ref, i) => (
                              <option key={i} value={ref.trim()} className="bg-gray-800 text-white" style={{ backgroundColor: isDark ? '#0f0f14' : undefined }}>
                                {ref.trim()}
                              </option>
                            ))}
                          </select>
                          ...
                          <div className="px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-gray-400 text-sm">
                            {formData.operation_reference || t('challengeExecution.notProvided', 'Não informada')}
                          </div>
                        )}
                      </div>
                      <div>
                        <label className="block text-xs text-gray-400 mb-1">{t('submissionReview.description', 'Descrição do Erro')} *</label>
                        <input
                          type="text"
                          value={err.description}
                          onChange={(e) => updateError(err.id, 'description', e.target.value)}
                          placeholder={t('submissionReview.describeError', 'Descreva o erro ocorrido...')}
            ...
            >
              {submitting ? (
                <>
                  ...
                  {t('challengeExecution.submitting', 'A submeter...')}
                </>
              ) : (
                <>
                  <Check className="w-5 h-5" />
                  {t('challengeExecution.submitChallenge', 'Submeter Desafio')}
                </>
              )}
            </button>
          </div>
