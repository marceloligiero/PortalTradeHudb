...
export default function LoginPage() {
  ...
  const { data: ssoStatus } = useQuery({
    queryKey: ['sso-status'],
    queryFn: () => api.get('/auth/microsoft/status').then(r => r.data as { enabled: boolean }),
    staleTime: Infinity,
    retry: false,
  });
      ...
      <div className="relative z-10 min-h-screen flex items-stretch pt-20 lg:pt-0">
        ...
        <div className="flex-1 flex items-center justify-center p-6 sm:p-10">
          <motion.div
            ...
            {/* Divider + SSO Microsoft (só aparece se MICROSOFT_CLIENT_ID estiver configurado no backend) */}
            {ssoEnabled && (
              <>
            ...
            {/* Separador antes do registo (quando SSO desativado) */}
            {!ssoEnabled && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
                className="flex items-center gap-4 my-8">
                <div className="flex-1 h-px bg-gray-200 dark:bg-white/[0.06]" />
