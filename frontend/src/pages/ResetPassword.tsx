...
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    const validateToken = async () => {
      try {
        ...
        setError(err.response?.data?.detail || t('auth.reset.tokenInvalid'));
      } finally {
        setValidating(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || t('auth.reset.errorResetting'));
    } finally {
      setLoading(false);
    }
  };
    ...
    if (validating) {
      return (
        <div className="text-center">
          ...
          <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>{t('auth.reset.validating')}</p>
        </div>
      );
    }
      ...
      return (
        <div className="text-center">
          ...
          <p className={`font-medium mb-6 ${
            ...
            {error || t('auth.reset.invalidLinkDesc')}
          </p>
      ...
      return (
        <div className="text-center">
          ...
          <h1 className={`text-3xl font-black tracking-tight mb-3 ${
            ...
            {t('auth.reset.passwordChanged')}
          </h1>
          <p className={`font-medium mb-6 ${
            isDark ? 'text-gray-400' : 'text-gray-600'
          }`}>
            {t('auth.reset.passwordChangedDesc')}
          </p>
          <Link to="/login">
            <Button variant="primary" size="lg" className="w-full">
      ...
      <>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {password && confirmPassword && password !== confirmPassword && (
            <p className="text-red-500 text-sm">{t('auth.reset.passwordsDontMatch')}</p>
          )}
  ...
  return (
    <div className={`min-h-screen relative overflow-hidden transition-colors duration-500 ${
      ...
      <PremiumNavbar isDark={isDark} onToggleTheme={toggleTheme} backTo={{ path: '/login', label: t('auth.backToLogin') }} />
