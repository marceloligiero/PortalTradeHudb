<#
setup-and-run-dev.ps1
Automate local dev setup using SQLite (avoids pyodbc/MSVC requirement).
Usage: Run from repository root in PowerShell (Admin not required):
  .\setup-and-run-dev.ps1
#>
...
Write-Host "Running dev setup (SQLite) — will create venv and install dev deps" -ForegroundColor Cyan

$repoRoot = Split-Path -Parent $MyInvocation.MyCommand.Definition
if ($PSScriptRoot) { $repoRoot = $PSScriptRoot } else { $repoRoot = Split-Path -Parent $MyInvocation.MyCommand.Definition }
...
if(-not (Test-Path (Join-Path $repoRoot 'backend'))){
    Write-Error "backend folder not found. Run this script from the repository root where the backend folder exists."; exit 1
}
...
if(Test-Path (Join-Path $repoRoot 'frontend')){
    ...
    try{
        Start-Process -NoNewWindow -FilePath "npm" -ArgumentList "run","dev" -WorkingDirectory (Join-Path $repoRoot 'frontend')
    } catch {
        Write-Host "Failed to start frontend (npm). You can run 'npm run dev' in the frontend folder manually." -ForegroundColor Yellow
    }
}
